...
Transform a feature request into a **comprehensive implementation plan** through systematic codebase analysis, external research, and strategic planning - designed for remote GitHub-first development workflow.

**Core Principle**: We do NOT write code in this phase. Our goal is to create a context-rich implementation plan that enables one-pass implementation success for AI agents working remotely via GitHub.

**Key Philosophy**: Context is King. The plan must contain ALL information needed for implementation - patterns, mandatory reading, documentation, validation commands, and GitHub workflow steps - so the execution agent succeeds on the first attempt and delivers a complete pull request.
...
- Read `.agents/PRD.md`(or search for the PRD) if it exists in the project
...
**GitHub Workflow:**
- **Branch Name**: `feature/<kebab-case-name>` (e.g., `feature/add-dark-mode`)
- **PR Title**: `<type>: <short description>` (e.g., `feat: Add dark mode toggle`)
- **Linked Issue**: #<issue-number> (if applicable)
...
### Phase 5: GitHub Workflow (CRITICAL - DO NOT SKIP)
...
- Link to this plan file in PR description
...
### Level 4: GitHub Workflow Validation
...
- <Include file additions, modifications, deletions>
...
- Implementation Plan: `.agents/plans/<plan-file>.md`
...
<Additional context, design decisions, trade-offs, GitHub workflow considerations>
```
...
- [ ] GitHub workflow steps included in tasks
...
- [ ] PR template provided for final deliverable
...
- [ ] GitHub workflow matches project practices
...
- [ ] Branch naming and PR title follow conventions
...
**GitHub Ready**: Plan includes all steps needed to create a complete pull request
...
**Confidence Score**: #/10 that execution will succeed on first attempt with complete PR

## Commit Plan to Feature Branch

**CRITICAL**: After creating the plan, commit it to a feature branch so execute can access it.
...
```bash
# Extract branch name from plan metadata
# Example: feature/add-contributing-section
git checkout -b <branch-name>
```

### Step 3: Stage and Commit Plan
```bash
git add .agents/plans/<plan-file>.md

git commit -m "docs: Add implementation plan for <feature-name>

Created comprehensive plan for implementing <feature description>.

Plan file: .agents/plans/<plan-file>.md
Ready for execution with: /command-invoke execute <branch-name>
...
**Log**:
```
✅ Plan committed to branch: <branch-name>
📁 Plan file: .agents/plans/<plan-file>.md
🚀 Ready for execution!

To execute this plan, run:
/command-invoke execute <branch-name> .agents/plans/<plan-file>.md
```

## Report

After creating the Plan and committing to branch, provide:

- Summary of feature and approach
- **Feature branch name** (e.g., `feature/add-contributing-section`)
- Full path to created Plan file (e.g., `.agents/plans/add-contributing-section.md`)
- **Execute command**: `/command-invoke execute <branch-name> .agents/plans/<plan-file>.md`
  - Example: `/command-invoke execute feature/add-contributing-section .agents/plans/add-contributing-section.md`
- Complexity assessment
- Key implementation risks or considerations
- Estimated confidence score for one-pass success
- **Proposed PR title** (from plan metadata)
